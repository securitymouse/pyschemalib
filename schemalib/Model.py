...
class Model(dict):
    ...
    def keys(self):
        ...
        l = self._getmembers()
        for i in l:
            if ((i not in self) and (type(i) == class and issubclass(i, Schema) and (i.required is True))) or (i in self):
                r += [i]
    ...
    def __iter__(self):
        ...
        while self.__iter_i < len(self.__iter_l):
            ...
            # Feign an object.
            if k not in self and (type(k) == class and issubclass(k, Schema) and (k.required is True)):
                yield (k, k.type())
            elif k in self:
                yield (k, self[k])
    ...
    def __getitem__(self, x):
        ...
        elif ((x not in self) and (type(x) == class and issubclass(x, Schema) and (x.required is True))):
            return x.type()
